...
import { HS256_SECRET, SYSTEM_USER_ID } from '../app/config';
import {
  UN_AUTH_ORIZATION,
...
const verifyAuth = async (ctx: Context, next: () => Promise<any>) => {
  try {
    ...
    const user = jwt.verify(token, HS256_SECRET);
    ctx.user = user;
    await next();
  } catch (error) {
...
export { verifyAuth, verifyPermission, parsingToken, verifyShowPermission };
