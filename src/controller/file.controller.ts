 ...
 * @LastEditTime: 2021-08-03 00:01:19
 * @LastEditors: mTm
 */
...
class FileController implements File.ControllerFile {
    async upload(ctx: File.FileCtx, next: () => Promise<any>) {
        try {
            ...
            if (ctx.user) {
                ...
                const result = await Promise.all(files.map((file: File.File) => service.fileCreate(file)))
                if (result) {
                    ctx.body = {
                        message: '图片上传成功'
                    }
